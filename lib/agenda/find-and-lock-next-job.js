...
module.exports = async function(jobName, definition) {
  ...
  if (s.topology.connections && s.topology.connections().length === 0 && !this._mongoUseUnifiedTopology) {
    ...
    if (self._useChannels) {
      const CHANNEL_WHERE_QUERY = {
        name: jobName,
        lockedAt: {$gt: lockDeadline},
        disabled: {$ne: true}
      };

      let values = await self._collection.distinct('channel', CHANNEL_WHERE_QUERY);
      debug('channels: %d channels locked when querying for job', values.length);
      if (values.length > 0) {
        JOB_PROCESS_WHERE_QUERY.$and.push({channel: {$nin: values}});
      }
    }
